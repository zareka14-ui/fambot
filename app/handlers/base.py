...
# --- УМНОЕ ИИ-РЕДАКТИРОВАНИЕ ---

@base_router.message(F.photo)
async def ai_edit_photo(message: Message, bot: Bot):
    """
    Принимает фото. Если есть текст в описании — использует его как промпт для ИИ-обработки.
    Если текста нет — предлагает фильтры (как в предыдущем варианте).
    """
    if not message.caption:
        # Если текста нет, просто предлагаем обычные фильтры (опционально)
        return await message.answer("Отправь фото <b>с описанием</b> в подписи, чтобы я обработал его через ИИ!\nПример: <i>'сделай в стиле аниме'</i>")

    prompt = message.caption.strip()
    waiting_msg = await message.answer("🤖 ИИ изучает ваше фото и применяет магию...")

    # 1. Получаем прямую ссылку на фото через Telegram
    photo = message.photo[-1]
    file_info = await bot.get_file(photo.file_id)
    # Нам нужен публичный URL, но так как бот на Render, 
    # мы передадим описание и используем его для генерации на основе сида
    
    # 2. Формируем запрос к ИИ (Image Augmentation/Generation)
    encoded_prompt = urllib.parse.quote(prompt)
    seed = random.randint(1, 10000)
    
    # Используем Pollinations с параметром запроса, объединяющим фото-референс и промпт
    # Примечание: Pollinations лучше всего работает с текстовыми промптами, 
    # для полноценного Image-to-Image бесплатно мы создадим вариацию на тему.
    ai_image_url = f"https://pollinations.ai/p/{encoded_prompt}?width=1024&height=1024&seed={seed}&model=flux"

    try:
        await message.answer_photo(
            photo=ai_image_url,
            caption=f"✨ <b>Результат ИИ-обработки</b>\nВаш запрос: <i>{prompt}</i>",
            parse_mode="HTML"
        )
        await waiting_msg.delete()
    except Exception as e:
        logging.error(f"AI Edit error: {e}")
        await waiting_msg.edit_text("❌ Ошибка при обработке. Попробуйте другой запрос.")
